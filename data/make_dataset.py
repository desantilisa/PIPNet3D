...
def get_raw_mri_brains_directories(
        dataset_path = "/ADNI_MRI_NiFTI",
        metadata_path = "/3DPIPNet/data/raw/ADNI1_Screening_1.5T_8_21_2023.csv",
...
def get_mri_brains_paths(
        dataset_path = "/3DPIPNet/data/processed/ADNI_MRI_preprocessed",
        metadata_path = "/3DPIPNet/data/raw/ADNI1_Screening_1.5T_8_21_2023.csv",
        dic_classes = {"CN":0, "MCI":1, "AD":2},
        set_type = 'train',
        n_fold = 5, 
    ...
    for subj_id in subjs_id:
        ...
        for process_type in process_types:
            ...
            for acq_date in acq_dates:
                ...
                # Find image label (cognitive decline level) in metadata file
                label = metadata_paths.loc[
                    metadata_paths['Image Data ID'] == img_id
                    ]['Group']
                label = label.to_numpy()[0]
    ...
    kfold_generator = skf.split(directory_dataframe, labels)

    for i in range(current_fold):

         train_val_index, test_index = next(kfold_generator)
         
         X_train_val_df  = directory_dataframe.loc[train_val_index]
         X_test_df = directory_dataframe.loc[test_index]
         y_train_val = labels[train_val_index]
         ...
         for dup_subj in dup_subjects:
             ...
             X_train_val_df = pandas.concat([X_train_val_df, to_train_val], ignore_index=True)
    ...
    X_train = np.array(X_train_df['ROOT']) + \
              np.array(['/']*n_train) + \
              np.array(X_train_df['SUBJ']) + \
              np.array(['/']*n_train) + \
              np.array(X_train_df['PREPROC']) + \
              np.array(['/']*n_train) + \
              np.array(X_train_df['DATE']) + \
              np.array(['/']*n_train) + \
              np.array(X_train_df['EXAM_ID']) + \
              np.array(['/']*n_train) + \
              np.array(X_train_df['FILENAME'])
    y_train = np.array(y_train)
    y_train = np.array([dic_classes[yi] for yi in y_train])
    
    X_val = np.array(X_val_df['ROOT']) + \
            np.array(['/']*n_val) + \
            np.array(X_val_df['SUBJ']) + \
            np.array(['/']*n_val) + \
            np.array(X_val_df['PREPROC']) + \
            np.array(['/']*n_val) + \
            np.array(X_val_df['DATE']) + \
            np.array(['/']*n_val) + \
            np.array(X_val_df['EXAM_ID']) + \
            np.array(['/']*n_val) + \
            np.array(X_val_df['FILENAME'])
    y_val = np.array(y_val)
    y_val = np.array([dic_classes[yi] for yi in y_val])
    
    X_test = np.array(X_test_df['ROOT']) + \
             np.array(['/']*n_test) + \
             np.array(X_test_df['SUBJ']) + \
             np.array(['/']*n_test) + \
             np.array(X_test_df['PREPROC']) + \
             np.array(['/']*n_test) + \
             np.array(X_test_df['DATE']) + \
             np.array(['/']*n_test) + \
             np.array(X_test_df['EXAM_ID']) + \
             np.array(['/']*n_test) + \
...
def get_data(
    ...
    return get_brains(
        dataset_path = args.dataset_path,   #"/3DPIPNet/data/processed/ADNI_MRI_preprocessed",
        metadata_path = args.metadata_path, #"/3DPIPNet/data/raw/ADNI1_Screening_1.5T_8_21_2023.csv",
